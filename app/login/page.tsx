...
import { GraduationCap, Mail, Lock, Github, Sparkles, Eye, EyeOff, KeyRound, Loader2 } from "lucide-react"
...
export default function LoginPage() {
  ...
  if (isLoading) {
    return (
      <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-slate-950 text-slate-100">
        <AnimatedBackground />
        ...
        >
          <div className="p-4 rounded-3xl bg-gradient-to-br from-primary to-accent text-primary-foreground shadow-2xl shadow-primary/30 animate-bounce">
            <GraduationCap className="h-10 w-10" />
          </div>
          <h1 className="text-2xl font-extrabold tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-foreground via-foreground/90 to-primary">
            AlumSphere
          </h1>
          <div className="flex items-center gap-1.5 mt-2 text-muted-foreground text-xs font-medium">
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-950 text-slate-100">
      ...
      >
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="text-center mb-6">
          <div className="inline-flex items-center gap-2 mb-3 cursor-pointer" onClick={() => router.push("/")}>
            <div className="p-3.5 rounded-2xl bg-gradient-to-br from-primary to-accent text-primary-foreground shadow-lg shadow-primary/20 hover:scale-105 transition-all">
            ...
            {verifying ? (
              <motion.div key="verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-amber-400/80 bg-amber-400/10 rounded-lg p-2">📬 Check your spam folder too!</p>
                </div>
                ...
                <button type="button" onClick={() => { setVerifying(false); setError("") }} className="w-full text-center text-xs text-muted-foreground hover:text-foreground transition-colors mt-4">
                  ← Back to sign up
                </button>
              </motion.div>
            ) : (
              <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex flex-col gap-3">
                    <Button type="button" disabled={isSubmitting} variant="outline" onClick={() => handleOAuth("google")} className="w-full bg-slate-900/30 border-border/50 hover:bg-slate-900/60 rounded-xl text-xs font-medium py-5 gap-2 disabled:opacity-50">
